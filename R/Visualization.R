...
plot(ages,salary) # should have same lengths, first = x-axis,
plot(salary,ages)
...
data()
airquality = datasets::airquality  # loads dataset to the memory
...
summary(airquality[,1]) # summary statistics for column 1 shows nulls as well
...
##################### Visualization
plot(airquality$Wind) # names as it is on y-axis
plot(airquality$Temp,airquality$Wind,type="p") #p = points defualt val
plot(airquality) #scatterplot of entire df
help(plot)
...
plot(airquality$Wind, 
     xlab = 'Ozone Concentration', #naming
     ylab = 'No of Instances', 
     main = 'Ozone levels in NY city',
     col = 'red',
     type='l')

plot(airquality,col='red') # scatterplot of entire dataset - all columns
...
barplot(airquality$Ozone, 
        main = 'Ozone Concenteration in air', # title
        ylab = 'ozone levels', 
        col= 'blue',
        horiz=T, # orientation
        axes=T)
...
boxplot(airquality$Wind,
        ...
        col="pink",
        border='red',
        horizontal=T) #default - vertical
boxplot.stats(airquality)
...
help(par)
par(mfrow=c(3,3),mar=c(1,2,1,2),  las=0, bty="o") # now can see 9 graphs at a time
...
max(airquality$Wind)
min(airquality$Temp)
min(airquality$Solar.R) # shows NA coz has null values
airquality$Solar.R
...
max(airquality$Solar.R,na.rm=T) # removes NA values and gives max
...
plot(density(airquality$Wind)) #histogram is joined lines
